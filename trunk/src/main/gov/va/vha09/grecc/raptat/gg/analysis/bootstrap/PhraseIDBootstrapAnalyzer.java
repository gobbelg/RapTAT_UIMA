...
/********************************************************
 * Instances of this class test the performance of the RapTAT application with respect to its
 * ability to identify phrases for annotation and map them to the proper concepts as defined by a
 * user. The analysis score returned includes both exact and partial phrase matching, which
 * indicates the ability to identify token sequences for annotation. Exact matches overlap by the
 * same, exact number of tokens. Partial matches overlap by 1 or more tokens, so an exact match is
 * also a partial match. Lastly, it determines the ability of the tool to map identified phrases to
 * correct concepts. If a phrase is a partial phrase match and maps to the correct concept, it is
 * considered a "true positive." If a phrase is a partial match but maps to the incorrect concept,
 * it is scored as a "false positive" for the incorrect concept and a "false negative" for the
 * concept that the tool should have mapped the phrase to. All phrases that are completely missed by
 * the phrase matching part of the tool are considered "false negatives" for the concept they match
 * to. Similarly, all phrases that are identified but should not map to anything are considered
 * "false positives."
 *
...
public class PhraseIDBootstrapAnalyzer {
  private class BootstrapSampler<T extends Object> {
    ...
    /*************************************************************
     * Create a hash in which the first element hashes to the a string array containing full paths
     * to the text document and the xml file
     *
     * @param theElements
     * @param theDirectory
     *
    ...
    /*************************************************************
     * Resets random sample generator to previous state before starting any runs
     *
   ...
   * strings.
   */
  protected final static int invertTokenSequenceMask = 1;

  protected final static int lemmatizationMask =
      2 * PhraseIDBootstrapAnalyzer.invertTokenSequenceMask;
  protected final static int posMask = 2 * PhraseIDBootstrapAnalyzer.lemmatizationMask;
  protected final static int stopWordsMask = 2 * PhraseIDBootstrapAnalyzer.posMask;
  protected final static int xtremeMask = 2 * PhraseIDBootstrapAnalyzer.stopWordsMask;
  protected final static int sevenMaxTokensMask = 2 * PhraseIDBootstrapAnalyzer.xtremeMask;
  protected final static int tenMaxTokensMask = 2 * PhraseIDBootstrapAnalyzer.sevenMaxTokensMask;

  private final static long seedSetting = 18492049756L;
