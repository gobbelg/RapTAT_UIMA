...
/********************************************************
 * This is a subclass of TokenSequnceID. This class is designed to carry out its training using
 * reverse ordering of the tokens in an annotated sequence to build a LabeledHashTree, so tokens at
 * the end of the annotated sequence are the first, rather than the last, elements in the
 * LabeledHashTree instance.
 *
...
public class ReverseProbabilisticSequenceIDTrainer extends ProbabilisticSequenceIDTrainer {
   ...
   * up to MAXIMUM_ELEMENTS_DEFAULT in length. Everything is stored in reverse order. The algorithm
   * works by evaluating each token in the document to see whether it is the end of a labeled
   * phrase. By going from the end of the document to the beginning, and by storing the positions of
   * the labeled phrases in a sorted array, we can quickly determine which tokens are at the end of
   * a labeled phrase and which are not. If a token is not the end of a labeled phrase, we can just
   * add the sequence ending with that token to the unlabeledHashTree for the document.
   *
