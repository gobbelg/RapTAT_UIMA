...
public class SQLAnnotationWriter {
  ...
  public void retrieveXMl() throws SQLDriverException {
    ...
    SQLAnnotationRetriever retriever =
        new SQLAnnotationRetriever(__dbConnectionObject, groupBatchSize);
    ...
    try {
      while ((annotationGroupBatch = retriever.getNextAnnotationGroupBatch(idIndex)) != null) {
        writeBatch(annotationGroupBatch);
        setNextExportDirectory();
  ...
  public void setNextExportDirectory() {
    ...
    exporter.setResultFolder(resultFolder);

  }


  public void writeBatch(Set<AnnotationGroup> annotationGroupBatch) throws ExporterException {
    for (AnnotationGroup annotationGroup : annotationGroupBatch) {
      boolean correctOffsets = true;
      boolean insertPhraseStrings = false;

      /*
       * pathtoSourceXMLFilesFolder is already implicitly provided by the RaptatDocument field of
       * annotationGroup, so we remove it from here.
       */
      ...
      exporter.exportAnnotationGroup(exportAnnonationGroupParameter);
    }
    ...
    SwingUtilities.invokeLater(new Runnable() {
      ...
      public void run() {
        ...
        DbConnectionObject dbConnectionObject =
            new DbConnectionObject(server, database, username, password);
