...
public class NBEvaluator extends ConceptMapEvaluator {
  ...
  protected boolean countNulls = Constants.USE_NULLS_START;
  protected boolean convertTokensToStems = Constants.USE_TOKEN_STEMMING;
  protected boolean addPOSToTokens = Constants.USE_PARTS_OF_SPEECH;
  protected boolean invertTokenSequence = Constants.INVERT_TOKEN_SEQUENCE;
  protected boolean removeStopWords = Constants.REMOVE_STOP_WORDS;
  protected int maxElements = Constants.MAX_TOKENS_DEFAULT;
  ...
  /**********************************************
   * This is a constructor used by an AnnotationFinder instance which handles all options such as
   * stemming and POS, so these don't need to be set as for other constructors, and not stemming or
   * pos tagging is needed.
   *
   * @param nbSolution
   * @param inFilter
   *
  ...
  /****************************************************************
   * Test the Naive Bayes network by seeing how well it maps test data to already mapped concepts
   *
  ...
  public List<String[]> evaluateSolution(DataImportReader testDataReader)
    ...
    if (durationList != null) {
      ...
      GeneralHelper.writeVectorAtInterval(evalTimeFileName, durationList,
          Constants.PHRASE_REPORT_INTERVAL);
    }
  ...
  public List<RaptatPair<List<RaptatToken>, String>> getConcepts(
    ...
    try {
      ...
      while (sequenceIterator.hasNext()) {
        ...
        if (!theConcept.equals(Constants.NULL_ELEMENT) || includeNulls) {
  ...
  public List<List<RaptatAttribute>> getMappedAttributes(List<String[]> phrases,
    ...
    List<List<RaptatAttribute>> resultAttributes =
        new ArrayList<>(phrases.size() * Constants.MAX_EXPECTED_ATTRIBUTES_PER_CONCEPT);
    ...
    try {
      while ((curData = testDataReader.getNextData()) != null) {
        if (curData.length > 1) {
          ...
          while (i < curData.length - 1 && !curData[i].equals(Constants.NULL_ELEMENT)) {
            i++;
          }
  ...
  private Vector<Long> getMatches(DataImportReader testDataReader)
    ...
    if (Constants.TRACK_DURATION) {
      // Add 1 to calculated size for easy handling of uneven divisions
      durationList = new Vector<>(
          1 + (int) testDataReader.getReaderLength() / Constants.PHRASE_REPORT_INTERVAL);
    }
      ...
      while (curData != null) {
        if (curData.length > 1) {
          ...
          if (phraseTokens != null && phraseTokens.length > 0) {
            ...
            if (Constants.TRACK_DURATION && curPhrase % Constants.PHRASE_REPORT_INTERVAL == 0) {
  ...
  private Vector<Long> getMatches(List<String[]> testDataSource, int startTokensPosition)
    ...
    if (Constants.TRACK_DURATION) {
      ...
      durationList = new Vector<>(1 + testDataSource.size() / Constants.PHRASE_REPORT_INTERVAL);
    }
    ...
    try {
      while (dataIterator.hasNext()) {
        ...
        if (curData.length > 0) {
          ...
          // column
          int i = startTokensPosition;
          while (!curData[i].equals(Constants.NULL_ELEMENT) && i < curData.length - 1) {
            i++;
          }
  ...
  private Vector<Long> getMatchesUseNulls(List<String[]> testDataSource, int startTokensPosition)
    ...
    if (Constants.TRACK_DURATION) {
      durationList = new Vector<>(1 + testDataSource.size() / Constants.PHRASE_REPORT_INTERVAL); // Add
      // 1
      // to
      // calculated
      // size
