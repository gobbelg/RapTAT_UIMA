...
/********************************************************
 * This tree is used as the primaryLabeledTree during training using XTREME_MOD training. It extends
 * the LabeledSubsequenceHashTree in that it does the scoring slightly differently. As for that
...
public class ModifiedSubsequenceHashTree extends LabeledSubsequenceHashTree {
  ...
  /*************************************************************
   * Adds the token string or string stems of the current phrase to this tree. Unlike the superclass
   * LabeledHashTree method, this method labels everything up to the last token in a sequence,
   * including each subsequence, with the length of the sequence determining how much score to give
   * it, with the score equal to the sequence positions divided by the length multiplied by the base
   * score. The maximum score is set so that the subsequence score is always an integer. It differs
   * from the LabeledSubsequenceHashTree parent class in that when an annotated phrase is added to a
   * tree, when subsequences are closer to the end of the actual phrase, they get more weight in
   * terms of labeling. So, if the phrase was "big red dog", "big" as the first term in the tree
   * would get less label value than "dog" as the first term in the tree, because dog is closer to
   * and, in fact is the end of the phrase.
   *
  ...
  public List<RaptatPair<List<RaptatToken>, Integer>> addLabeledPhrase(
    ...
    for (int startTokenIndex = 0; startTokenIndex < sequenceSize; startTokenIndex++) {
      ...
      int curMaxScore = Constants.MAX_LABELED_SCORE * curMaxLength / sequenceSize;
