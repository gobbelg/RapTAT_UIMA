...
public class PhraseTree extends HashTree<PhraseTree>
   ...
   * @author Glenn Gobbel - Jul 17, 2014
   */
  public void addPhrasesAsListOfLists(List<List<String>> thePhrases, Mark marker) {
   ...
   * @author Glenn Gobbel - Jul 17, 2014
   */
  public void addPhrasesAsSortableLists(List<SortableStringArrayList> thePhrases,
      Mark marker) {
    for (List<String> curPhrase : thePhrases) {
      this.addPhrase(curPhrase, marker, 0);
    }
  ...
  /**
   * Determines whether a phrase, provided as a String array, exists as a sequence in this
   * PhraseTree instance and has a Mark instance equivalent to the parameter mark at the end.
   *
   *
   *
   * @param phrase
   * @param mark
   * @return Returns a boolean, which will be false for phrases that are null, zero-length, or do
   *         not exist as a sequence with the Mark mark at the end. Otherwise returns true.
   */
  public boolean containsMarkedPhrase(String[] phrase, Mark mark) {

    if (phrase == null || phrase.length == 0) {
      return false;
    }

    Optional<PhraseTree> curTree = getPhraseTree(phrase);
    return curTree.isEmpty() ? false : curTree.get().phraseTag == mark;
  }


  /**
   * Determines whether a phrase, provided as a String array method parameter, exists as a sequence
   * or subsequence in this PhraseTree instance
   *
   * @param phrase
   * @return
   */
   ...
   * @return
   */
  public int phraseLengthAtIndexIgnoreCase(String[] tokenStrings, int startIndex,
      Mark markToFind) {
    if (tokenStrings == null || startIndex >= tokenStrings.length) {
      return 0;
    }
  ...
  /**
   * Gets the PhraseTree instances hashed to by the strings in the String array instance, 'phrase',
   * which is provided as a parameter to the method.
   *
   * Setting the String [] parameter 'phrase' to null will result in an error.
   *
   * @param phrase
   * @return
   */
  private Optional<PhraseTree> getPhraseTree(String[] phrase) {
    PhraseTree curTree = this;
    int curIndex = 0;
    ...
    return Optional.ofNullable(curTree);
  }
  ...
  public static void main(String[] args) {
    /*
     * Code to test containsMarkedPhrase() vs containsPhrase() PhraseTree methods
     */

    /*
     * Create PhraseTree instance with some random phrases
     */
    PhraseTree pt = new PhraseTree();
    List<List<String>> listOfLists = new ArrayList<>();
    List<String> simpleList = new ArrayList<String>();
    simpleList = Arrays.asList("dog", "ball");
    listOfLists.add(simpleList);

    simpleList = Arrays.asList("run", "dog", "tree");
    listOfLists.add(simpleList);

    simpleList = Arrays.asList("the", "cat");
    listOfLists.add(simpleList);

    simpleList = Arrays.asList("elephant");
    listOfLists.add(simpleList);

    pt.addPhrasesAsListOfLists(listOfLists, Mark.DefaultMark.NONE);

    /*
     * Now test other phrases to see if they are in the PhraseTree pt by using methods
     * containsMarkedPhrase() or containsPhrase()
     */
    String[] testArray;

    testArray = new String[] {"dog", "tree"};
    System.out.print("'" + String.join(" ", testArray) + "' found in PhraseTree with Mark:");
    System.out.println(pt.containsMarkedPhrase(testArray, Mark.DefaultMark.NONE));
    System.out.print("'" + String.join(" ", testArray) + "' found in PhraseTree without Mark:");
    System.out.println(pt.containsPhrase(testArray));
    System.out.println();

    testArray = new String[] {"run", "dog", "tree"};
    System.out.print("'" + String.join(" ", testArray) + "' found in PhraseTree with Mark:");
    System.out.println(pt.containsMarkedPhrase(testArray, Mark.DefaultMark.NONE));
    System.out.print("'" + String.join(" ", testArray) + "' found in PhraseTree without Mark:");
    System.out.println(pt.containsPhrase(testArray));
