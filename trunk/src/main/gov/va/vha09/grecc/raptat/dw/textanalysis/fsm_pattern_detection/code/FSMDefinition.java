...
public class FSMDefinition {
  ...
  final private MatchAction NoMatchAction =
   ...
   */
  public void define(State current_state, MatchPattern match_pattern, State new_state,
      MatchAction action) throws FSMElementException {
   ...
   * @throws FSMElementException the FSM element exception
   */
  public void label(RaptatToken token, MatchType match_type) throws FSMElementException {
    if (this.__fsm == null) {
    ...
    for (MatchPattern matchPattern : this.__match_patterns.get_elements()) {
      if (matchPattern.matches(token, this.__raptat_token_text_callback, match_type) != null) {
        found_pattern = matchPattern;
        break;
      }
    }

    int action = this.__fsm.update(this.__match_patterns.get_index(found_pattern));

    MatchAction match_action = this.__match_actions.get(action);
