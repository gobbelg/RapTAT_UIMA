...
/********************************************************
 * Class created to extend the CSVReader class. This one will tokenize the column that contains a
 * string that is untokenized. Its use is with training or running the concept mappiing system,
 * where the phrase is not already tokenized.
 *
...
public class TokenizingTextReader extends TextReader {
  ...
  /****************************************************************
   * Returns the number of tokens in the phrase if the position in the data where phrase tokens
   * starts is known. This overrides the method iin DataImportReader, because with the
   * CSVReaderAndTokenizer, the nunber of phrase tokens actually in the data depends on the line,
   * but an instance of this class always returns a phrase with maxTokens elements when
   * getNextData() is called. However, returns -1.
   *
   * @return int
   ****************************************************************/
  @Override
  ...
  private String[] tokenize(String[] dataItems) {
    if (dataItems.length > 0) {
      ...
      while (i < tokensIndex + this.maxTokens) {
        result[i++] = Constants.NULL_ELEMENT;
      }
