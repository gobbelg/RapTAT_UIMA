...
public class BOWSequenceIDCalculator {
  ...
  /*************************************************************
   * Identify the first and last index of sequences within a sentence that have at least one token
   * that has occurred as part of an annotated sequence. The first token in the sequences are stored
   * in a hashmap where the index value of the start maps to a set of potential ending indices.
   * These are used to select sublists to evaluate for likelihood of annotation. Note that the
   * sublist method includes the first index to the next to last index only when used in the form
  ...
  public ConcurrentHashMap<Integer, HashSet<Integer>> getSequencesForEvaluation(
    ...
    ConcurrentHashMap<String, OccurrenceSet> conditionalsForEvaluation =
        this.tokenConditionals[Constants.BOW_SEQUENCE_ID_PHRASE_INDEX];
    ...
    for (Integer curStartIndex : annotatedIndices) {
      ...
      // annotation
      for (int i = curStartIndex + 1; i - curStartIndex <= Constants.MAX_TOKENS_DEFAULT
          && i < sentenceSize; i++) {
        curEndIndices.add(i);
      }
    ...
    for (Integer annotationIndex : annotatedIndices) {
      ...
      int startIndex = endIndex - Constants.MAX_TOKENS_DEFAULT;
  ...
  private double getDownstreamProbability(Integer downstreamStartIndex,
    ...
    if (downstreamStartIndex < sentenceSize) {
      ...
      for (int i = downstreamStartIndex, j = 0; i < sentenceSize
        ...
        if ((occSet =
            this.tokenConditionals[Constants.BOW_SEQUENCE_ID_POST_INDEX].get(curKey)) != null) {
          result += occSet.likelihood;
        } else {
          result += this.missingTokenLikelihood[Constants.BOW_SEQUENCE_ID_POST_INDEX];
        }
      }
    }
    return result;
  ...
  private double getPhraseProbability(List<String> phraseSequence) {
    ...
    for (String curTokenString : phraseSequence) {
      if ((occSet = this.tokenConditionals[Constants.BOW_SEQUENCE_ID_PHRASE_INDEX]
          .get(curTokenString)) != null) {
        result += occSet.likelihood;
      } else {
        result += this.missingTokenLikelihood[Constants.BOW_SEQUENCE_ID_PHRASE_INDEX];
      }
    }
  ...
  private double getUpstreamProbability(Integer phraseStartIndex, List<String> inputSentence) {
    ...
    if (phraseStartIndex != 0) {
      ...
      for (int i = phraseStartIndex - 1, j = 0; i >= 0 && j < this.tokenWindowWidth; i--, j++) {
        String curKey = inputSentence.get(i);
        if ((curSet =
            this.tokenConditionals[Constants.BOW_SEQUENCE_ID_PRIOR_INDEX].get(curKey)) != null) {
          result += curSet.likelihood;
        } else {
          result += this.missingTokenLikelihood[Constants.BOW_SEQUENCE_ID_PRIOR_INDEX];
        }
      }
    }
    return result;
