...
public class LabeledSubsequenceHashTree extends LabeledHashTree {
   ...
   * it, with the score equal to the sequence positions divided by the length multiplied by the
   * maximum achievable score. The maximum score is set so that the subsequence score is always an
   * integer.
   *
  ...
  public List<RaptatPair<List<RaptatToken>, Integer>> addLabeledPhrase(
    ...
    for (int startTokenIndex = 0; startTokenIndex < sequenceSize; startTokenIndex++) {
      for (int endTokenIndex =
        ...
        int score = subSequence.size() * Constants.MAX_LABELED_SCORE / sequenceSize;
