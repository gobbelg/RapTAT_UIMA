  ...
  private class CreateDBListener implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      try (SQLDriver driver = new SQLDriver(serverName, dbName, userName, password)) {
        boolean created = driver.createDatabase();

        if (created) {
          driver.executeQueryFile(".\\NLP DB\\NLP DB Create User.sql", dbName);
          // Creating a set of table with prefix and another set of
          // table without prefix
          driver.createTablesFromQueryFile(".\\NLP DB\\NLP DB Create Table.sql", prefix);
