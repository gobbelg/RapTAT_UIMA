...
/************************************************************
 * DataImporter - Creates a DataImportReader that can be used to import data from a data source into
 * RapTAT. We include a DataImportReader as a way of allowing this DataImporter to import data of
...
public class DataImporter {
  ...
  public DataImporter(String thePrompt, boolean getStartTokensPostion) {
    ...
    fc.addChoosableFileFilter(new MultiFileFilter(Constants.IMPORTABLE_FILETYPES));
    ...
    if (fileChosen == JFileChooser.APPROVE_OPTION) {
      ...
      if (fileName.endsWith(Constants.IMPORTABLE_FILETYPES[0])) {
        int tokenizeOption =
        ...
        this.theReader.setFileType(Constants.IMPORTABLE_FILETYPES[0]);
      } else if (fileName.endsWith(Constants.IMPORTABLE_FILETYPES[1])) {
        int tokenizeOption =
            JOptionPane.showConfirmDialog(null, "Tokenize data?", "", JOptionPane.YES_NO_OPTION);
        if (tokenizeOption == JOptionPane.YES_OPTION) {
          this.theReader = new TokenizingTextReader(selectedFile, getStartTokensPostion);
        } else {
          this.importFileType = Constants.IMPORTABLE_FILETYPES[2];
          this.theReader = new TextReader(selectedFile, getStartTokensPostion);
        }
        this.theReader.setFileType(Constants.IMPORTABLE_FILETYPES[1]);
      } 
//      else if (fileName.endsWith(Constants.IMPORTABLE_FILETYPES[2])) {
//        this.theReader = new KnowtatorReader(selectedFile);
//        this.theReader.setFileType(Constants.IMPORTABLE_FILETYPES[2]);
//      }
      this.theReader
