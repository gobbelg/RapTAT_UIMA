...
public class PhraseIDCrossValidationSampler
   ...
   * @return
   */
  public boolean getSamplingData(List<SchemaConcept> validSchemaConcepts,
    ...
    if (testingOption != JOptionPane.CANCEL_OPTION) {
      return addCrossValidationSamples(dataReader, validSchemaConcepts, trainingConcepts,
  ...
  private boolean addCrossValidationSamples(CSVReader dataReader,
      List<SchemaConcept> validSchemaConcepts, HashSet<String> trainingConcepts,
    ...
    while ((nextDataSet = dataReader.getNextData()) != null) {
      ...
      if (nextDataSet.length > 2) {
        addSample(nextDataSet, validSchemaConcepts, trainingConcepts, scoringConcepts,
  ...
  /*************************************************************
   * Imports the text documents, creating a RaptatDocument instance, and the annotations of that
   * document. These are stored in the fields sampleGroups, trainingOnlyGroups, and testGroups.
   *
   ...
   * @param trainingConceptNameSet
   * @param scoringConcepts
   * @param importErrorPrinter
   *************************************************************/
  private void addSample(String[] dataElements, List<SchemaConcept> schemaConceptList,
      HashSet<String> trainingConceptNameSet, HashSet<String> scoringConcepts,
    ...
    // [[ Logger Folding
    /*
     * Note it doesn't appear that there will be any active sentences at this point. Since this is
     * logging code, perhaps it does not matter.
     */
    ...
    List<AnnotatedPhrase> trainingAnnotations = this.theImporter.importAnnotations(trainingXMLPath,
        trainingDocPath, trainingConceptNameSet, schemaConceptList, importErrorPrinter);
