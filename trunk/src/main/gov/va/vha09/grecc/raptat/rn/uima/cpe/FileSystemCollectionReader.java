...
package src.main.gov.va.vha09.grecc.raptat.rn.uima.cpe;
...
public class FileSystemCollectionReader extends CollectionReader_ImplBase {
  ...
  private void addFilesFromDir(File dir) {
	//Richard Noriega 10-21-21 SilkCA
	String nextDocName = BridgeClass.nextDocName;
    File[] files = dir.listFiles();
    for (int i = 0; i < files.length; i++) {
      if (!files[i].isDirectory() && files[i].getName().contains(nextDocName)) {
        this.mFiles.add(files[i]);
        return;
      } else if (this.mRecursive && files[i].isDirectory()) {
