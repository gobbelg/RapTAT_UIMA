...
/*************************************************
 * A class used for writing out a single object or a list of object instances. Objects, single or as
 * a list, can be appended to the end of an existing file with calls to the append() methods. Calls
 * to write() will overwrite files of the same name if they already exist.
 *
 * @author Glenn Gobbel - February 16, 2012
 *************************************************/
public class ListOutputStream<E extends Serializable> {
  /*******************************************************************
   * This class is needed to make sure that no header gets written when appending the stream
   *
  ...
  private String outStreamMarker = Constants.LIST_OUTPUTSTREAM_MARKER;
  ...
  /****************************************************
   * Creates a new ObjectOutputStreamer. Objects will be save into the file specified in the path
   * given by fileName.
   *
   * @param fileName
   ****************************************************/
      ...
      try {
        ...
        if (outFile.exists()) {
          outStream = new AppendableObjectOutputStream(new BufferedOutputStream(
              new FileOutputStream(outFile, true), Constants.DEFAULT_BUFFERED_STREAM_SIZE));
          if (outStream != null) {
    ...
    if (this.outStreamName != null) {
      try {
        outStream = new ObjectOutputStream(new BufferedOutputStream(
            new FileOutputStream(this.outStreamName), Constants.DEFAULT_BUFFERED_STREAM_SIZE));
        if (outStream != null) {
          outStream.writeObject(this.outStreamMarker);
          outStream.writeObject(theObject);
        }
