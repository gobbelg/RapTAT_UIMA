...
/********************************************************
 * OffsetConverter interface implementation that converts beginning and end character offsets from
 * UTf-8 for OpenNLP to eHOST (or Knowtator XML)
 *
...
public class UTFToAnnotatorOffsetConverter implements OffsetConverter {
   ...
   * @param sourceFile
   *
   * @author Glenn Gobbel - Sept 18, 2018
   * @param annotationSource
   **********************************************/
  public UTFToAnnotatorOffsetConverter(File sourceFile, AnnotationApp annotationSource) {
    this.offsetCorrections = getOffsetCorrections(sourceFile, annotationSource);
  }
   ...
   * @param annotationSource
   **********************************************/
  public UTFToAnnotatorOffsetConverter(String rawInputText, AnnotationApp annotationSource) {
    this.offsetCorrections = getOffsetCorrections(rawInputText, annotationSource);
  }
  ...
  /*************************************************************
   * The method changes the actual offsets within the phrases passed to the method.
   *
   * @param allAnnData
   *
   *
  ...
  /*************************************************************
   * OVERRIDES PARENT METHOD Take a begin and end offset as integers and return a pair of Integer
   * objects with the left object being the converted begin offset and the right object being the
  ...
  /*************************************************************
   * OVERRIDES PARENT METHOD Method to take a list of offsets, provided in pairs with the left
   * element of the pair the beginning offset and the right element of the pair the end offset, and
   * return another list with the convertted offsets provided in pairs in the same order as the
   * input list.
   *
   * @param offsetList
  ...
  /*************************************************************
   * OVERRIDES PARENT METHOD Take a pair, where left element = beginning offset and right element =
   * end offset, and return another pair with the left element giving the converted begin offset and
   * the right element giving the converted end offset.
   *
   * @param offsetsList
  ...
  /*************************************************************
   * Determine the offsets needed to generate the proper start and end offsets for an AnnotationApp
   * (e.g. Knowtator or eHost) based on a document that was analyzed by OpenNLP assuming UTF-8 type
   *
   ...
   * @param annotationSource
   *************************************************************/
  private IndexedTree<IndexedObject<Integer>> getOffsetCorrections(Object textSource,
      AnnotationApp annotationSource) {
    IndexedTree<IndexedObject<Integer>> offsetComparer =
        OffsetParser.calculateOffsetCorrectionsNew(textSource, annotationSource);
