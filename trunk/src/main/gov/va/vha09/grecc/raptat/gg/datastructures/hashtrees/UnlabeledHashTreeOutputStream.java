...
/*************************************************
 * A class used for writing out a single UnlabeledHashTree or a list of UnlabeledHashTree instances.
 * UnlabeledHashTrees, single or as a list, can be appended to the end of an existing file with
 * calls to the append() methods.
 *
...
public class UnlabeledHashTreeOutputStream implements HashTreeOutStreamer<UnlabeledHashTree> {
  ...
  /****************************************************
   * Creates a new UnlabeledHashTreeOutputStream. UnlabeledHashTrees will be save into the file
   * specified in the path given by fileName.
   *
   * @param fileName
   ****************************************************/
      ...
      try {
        if (this.outStreamFile.exists() && this.outStreamFile.length() > 0) {
          ...
          outStream = new ObjectOutputStream(new BufferedOutputStream(
              new FileOutputStream(this.outStreamFile), Constants.DEFAULT_BUFFERED_STREAM_SIZE));
        }
  ...
  /*************************************************************
   * OVERRIDES PARENT METHOD This does not do anything but is only included to conform to the
   * HashTreeOutStreamer interface. This class keeps its hash trees on disk, so writing them to disk
   * is not necessary.
   *
   *
