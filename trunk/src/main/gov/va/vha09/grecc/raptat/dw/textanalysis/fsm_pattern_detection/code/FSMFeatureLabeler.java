 ...
 */
public class FSMFeatureLabeler {
   ...
   */
  public void identify(List<RaptatToken> raptat_token_list) throws FSMElementException {
    this.identify(raptat_token_list, MatchType.Exact);
   ...
   */
  public void identify(List<RaptatToken> tokens, MatchType match_type) throws FSMElementException {
    for (RaptatToken token : tokens) {
      for (FSMDefinition fsm : this.__fsm_array) {
        if (fsm != null) {
          fsm.label(token, match_type);
        }
      }
    }
  }
   ...
   * @throws FSMElementException
   */
  public void identify(RaptatToken raptat_token, MatchType match_type) throws FSMElementException {
    List<RaptatToken> list = new ArrayList<>();
    list.add(raptat_token);
    this.identify(list, match_type);
