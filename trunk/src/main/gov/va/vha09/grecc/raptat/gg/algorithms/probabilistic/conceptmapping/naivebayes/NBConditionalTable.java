...
public class NBConditionalTable extends Hashtable<String, NBConceptLookupTable>
  ...
  public NBConditionalTable(int startCapacity) {
    super(startCapacity);
    this.modifiedElements = new StrPairSet(Constants.NB_UNIQUE_TOKEN_CONCEPT_PAIRS);
  ...
  /******************************************************************
   * Returns the value specified by two keys into the two dimensional table that backs this object.
   *
  ...
  /****************************************************************
   * Increment the occurrence table at the location specified by the first and second keys
   *
  ...
  public boolean increment(String tokenString, String theConcept) {
    ...
    if (containsKey(tokenString)) {
      ...
      conceptTable = new NBConceptLookupTable(Constants.NB_CONCEPTS_PER_ATTRIBUTE);
