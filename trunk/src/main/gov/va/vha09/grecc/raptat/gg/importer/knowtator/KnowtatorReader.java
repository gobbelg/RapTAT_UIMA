...
/************************************************************
 * Reads in a Knowtator Project file and determines maximum number of elements within all annotated
 * phrases in the file.
 *
...
public class KnowtatorReader extends DataImportReader {
  ...
  /****************************************************************
   * Supplies the next line of data in the file with each element generating one element of a String
   * array.
   *
  ...
  public String[] getNextData() {
    ...
    Pattern p = Pattern.compile(Constants.SPLITTER_REGEX);
  ...
  /****************************************************************
   * Supplies the next line of data in the file with each element generating one element of a String
   * array. This is the same as getNextData() and is included only to conform to the abstract
   * superclass that contains subclasses where this method is needed.
   *
   * @return String [] - Array containing data elements with the document source at index 0, the
   *         annotation at the last position and the tokens of the annotated phrase at the
   *         intervening indices.
  ...
  /****************************************************************
   * Resets reader so user can read starting at the first text source in the knowtator file
   ****************************************************************/
  @Override
  public void reset() {
    edu.stanford.smi.protege.model.Project project =
  ...
  private String[] padStringArray(String[] inArray) {
    ...
    for (int i = inArrayLength; i < this.numPhraseTokens + 2; i++) {
      returnArray[i] = Constants.NULL_ELEMENT;
    }
    return returnArray;
  }
