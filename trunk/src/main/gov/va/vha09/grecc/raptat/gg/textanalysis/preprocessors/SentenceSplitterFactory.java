...
public class SentenceSplitterFactory {
  ...
  private static void createInstance() {
    if (SentenceSplitterFactory.splitterInstance == null) {
      ...
      try {
        modelIn = SentenceSplitterFactory.class.getResourceAsStream("en-sent.bin");
        System.out.println("Loading sentence splitter model");
        theModel = new SentenceModel(modelIn);
        theModel.useTokenEnd();

        if (theModel != null) {
          SentenceSplitterFactory.splitterInstance = new SentenceDetectorME(theModel);
        }
      }

      catch (IOException e) {
        System.out.println("Unable to load sentence splitter model\n" + e);
        e.printStackTrace();
      }
