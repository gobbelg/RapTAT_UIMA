...
/************************************************************
 * Interface to assure that all the databases generated from training using the various machine
 * learning methods contain a saveToFile method to store solutions. Note that many of the boolean
 * fields, such as usePOS and useStems, are stored here because they are needed to determine token
 * processing durin application of the solution even though they are not directly used by the
 * instance of the ConceptMapSolution itself.
 *
...
public abstract class ConceptMapSolution implements Serializable {
  ...
  private boolean usePOS = Constants.USE_PARTS_OF_SPEECH;

  private boolean useStems = Constants.USE_TOKEN_STEMMING;

  private boolean removeStopWords = Constants.REMOVE_STOP_WORDS;

  private boolean invertTokenSequence = Constants.INVERT_TOKEN_SEQUENCE;

  private boolean useNulls = Constants.USE_NULLS_START;
