 ...
 */
public class AnnotationOutputAdapter extends RapTATDatabaseConnection implements IOutputAdapter {
  ...
  private static Counter exportCallCounter = new Counter();

  private File annotationSchemaFile;
   ...
   */
  public AnnotationOutputAdapter(IDataSourceConfiguration config, AuditAdapter auditAdapter,
      Logger logger, File schemaFile) {
    super(config, auditAdapter, logger);
    annotationSchemaFile = schemaFile;
   ...
   *
   * @param processedItem the result of text processing
   ...
   */
  @Override
  public void writeResult(RaptatResult processedItem) throws BaseException {

    RaptatAnnotationResult annotationResult = (RaptatAnnotationResult) processedItem;
    /*
     * Initialize the audit log with this stage.
     */
    processedItem.getItem().markWritingToDatabase();
     ...
     */
    processedItem.getItem().markWrittenToDatabase();
   ...
   */
  private void export(RaptatAnnotationResult annotationResult)
      throws WriteException, DataSourceException {

    AnnotationGroup annotationGroup = annotationResult.getAnnotationGroup();

    IDataSourceConfiguration config = getConfig();

    boolean correctOffsets = config == null ? DEFAULT_CORRECT_OFFSETS : config.getCorrectOffsets();
    boolean insertPhraseString =
        config == null ? DEFAULT_INSERT_PHRASE_STRING : config.getInsertPhraseString();
    ...
    SQLExporter sqlExporter = new SQLExporter(annotationApp, exportCallCounter.next().toString(),
        config.getSqlExportSchemaName(), annotationSchemaFile.getAbsolutePath(), sqlDriver,
    ...
    try {
       ...
       */
      // String documentPath;
      // documentPath = annotationResult.getItem().getDocumentPath().orElse("NoDocumentPath");

      SqlExportAnnotationGroupParameter exportAnnotationGroupParameter =
          new SqlExportAnnotationGroupParameter(annotationGroup, correctOffsets,
              insertPhraseString /* , documentPath */);
      sqlExporter.exportAnnotationGroup(exportAnnotationGroupParameter);
    } catch (Exception e) {
      getLogger().error(e);
  ...
  public static void main(String[] argv) {
    ...
    /** The Constant logger. */
    final Logger logger = Logger.getLogger(AnnotationOutputAdapter.class);
