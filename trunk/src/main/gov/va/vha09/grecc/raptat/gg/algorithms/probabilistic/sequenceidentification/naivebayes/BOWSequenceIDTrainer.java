...
public class BOWSequenceIDTrainer extends SequenceIDTrainer {
  ...
  /**********************************************
   * Constructor called when a solution already exists that needs to be updated
   *
  ...
  /**********************************************
   * Constructor called when there is no existing solution
   *
   * @param trainingOptions
   *
   * @author Glenn Gobbel - Oct 12, 2012
  ...
  /*************************************************************
   * The algorithm will first add all tokens as unannotated. Then, it will go through the list of
   * annotated phrases and tokens, decrementing the "non-annotated" values and decrementing the "not
   * upstream of an annotation" values as well as incrementing their annotated occurrence values and
   * the upstream occurrence values of the appopriate tokens. We do it this way to avoid having to
   * check multiple times whether a token is annotated, upstream, or downstream. We assume most are
   * not.
   *
   * @param theSentence
  ...
  private void addSentenceWithAnnotations(AnnotatedPhrase theSentence,
    ...
    this.theSolution.updateAnnotated(allPhraseTokens, Constants.BOW_SEQUENCE_ID_PHRASE_INDEX);
    this.theSolution.updateAnnotated(allUpstreamTokens, Constants.BOW_SEQUENCE_ID_PRIOR_INDEX);
    this.theSolution.updateAnnotated(allDownstreamTokens, Constants.BOW_SEQUENCE_ID_POST_INDEX);
    ...
    for (RaptatToken curToken : sentenceTokens) {
      ...
      if (!allPhraseTokens.contains(curToken) && !RaptatToken.startOrEndToken(curToken)) {
        this.theSolution.incrementUnannotated(tokenString, 1,
            Constants.BOW_SEQUENCE_ID_PHRASE_INDEX);
      }
      if (!allUpstreamTokens.contains(curToken)) {
        this.theSolution.incrementUnannotated(tokenString, 1,
            Constants.BOW_SEQUENCE_ID_PRIOR_INDEX);
      }
      if (!allDownstreamTokens.contains(curToken)) {
        this.theSolution.incrementUnannotated(tokenString, 1, Constants.BOW_SEQUENCE_ID_POST_INDEX);
      }
    }
  }
  ...
  /*************************************************************
   * Determine the token strings that are downstream from a phrase within a sentence and return the
   * token strings as a list
   *
  ...
  /*************************************************************
   * Creates a mapping so that every sentence can be hashed to return the phrases within the
   * sentence that are annotated
   *
   * @param theSentences
   * @param theAnnotations
  ...
  /*************************************************************
   * Determine the token strings that are upstream from a phrase within a sentence and return the
   * token strings as a list
   *
   * @param sentenceTokens
   * @param sentenceTokenStrings
