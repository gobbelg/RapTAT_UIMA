...
import src.main.gov.va.vha09.grecc.raptat.gg.core.Constants.OptimismCorrectionMethod;
import src.main.gov.va.vha09.grecc.raptat.gg.core.Constants.PhraseIDSmoothingMethod;
import src.main.gov.va.vha09.grecc.raptat.gg.core.Constants.TSFinderMethod;
...
/************************************************************
 * OptionManager - Singleton class that keeps track of which options have been set and allows for
 * changing of options
 *
...
public final class OptionsManager {
  private final class AnalysisOptionsWindow extends JFrame implements ActionListener {
    ...
    private AnalysisOptionsWindow(Point position, Dimension dim) {
      ...
      windowWidth = windowWidth < Constants.ANALYSIS_OPTIONS_MIN_WIDTH ? windowWidth
          : Constants.ANALYSIS_OPTIONS_MIN_WIDTH;
      windowHeight = windowHeight < Constants.ANALYSIS_OPTIONS_MIN_HEIGHT ? windowHeight
          : Constants.ANALYSIS_OPTIONS_MIN_HEIGHT;
    ...
    private JPanel buildGroupBySelectionPanel() {
      ...
      this.groupByColumnSelector = new JComboBox(Constants.GROUP_BY_COLUMN_OPTIONS);

      this.groupByColumnSelector
          .setSelectedItem(Constants.GROUP_BY_COLUMN_OPTIONS[OptionsManager.this.grpByColNum]);
    ...
    /****************************************************************
     * Build panel to save options settings
     *
     * @return Component
     ****************************************************************/
    ...
    /****************************************************************
     * //Save all the settings currently set in the options window dialog
     *
     ****************************************************************/
  ...
  private final class ConceptMappingOptionsWindow extends JFrame implements ActionListener {
    ...
    public ConceptMappingOptionsWindow(Point position, Dimension dim) {
      int windowWidth = (int) dim.getWidth() * 2 / 3;
      int windowHeight = (int) dim.getHeight() * 2 / 3;
      windowWidth = windowWidth < Constants.MAP_OPTIONS_MIN_WIDTH ? windowWidth
          : Constants.MAP_OPTIONS_MIN_WIDTH;
      windowHeight = windowHeight < Constants.MAP_OPTIONS_MIN_HEIGHT ? windowHeight
          : Constants.MAP_OPTIONS_MIN_HEIGHT;
      this.setSize(windowWidth, windowHeight);
    ...
    private JPanel buildExportTargetPanel() {
      ...
      this.exportTargetSelector = new JComboBox(Constants.EXPORTABLE_FILETYPES);
  ...
  private final class TestOptionsWindow extends JFrame implements ActionListener {
    ...
    private TestOptionsWindow(Point position, Dimension dim) {
      ...
      // Make sure window is big enough
      windowWidth =
          windowWidth < Constants.TEST_OPTIONS_MIN_WIDTH ? Constants.TEST_OPTIONS_MIN_WIDTH
              : windowWidth;
      windowHeight =
          windowHeight < Constants.TEST_OPTIONS_MIN_HEIGHT ? Constants.TEST_OPTIONS_MIN_HEIGHT
              : windowHeight;
    ...
    /****************************************************************
     * Set a button group so all elements are not selected and disabled
     *
     * @param theGroups
     ****************************************************************/
    ...
    /****************************************************************
     * buildCmpPanel - deletes words from a phrase during testing to see if the matching improves
     *
     * @return JPanel
     ****************************************************************/
    private JPanel buildCmpPanel(String panelTitle) {
      ...
      for (int i = 0; i <= Constants.MAX_WORD_CHANGE; i++) {
        curButton = new JRadioButton(Constants.NUMSTRINGS[i]);
    ...
    /****************************************************************
     * Build panel to determine how many virtual words to expand or delete from a phrase during
     * testing
     *
     * @return JPanel
     ****************************************************************/
    private JPanel buildExpCmpPanel() {
      ...
      expCmpPanel.add(buildExpPanel(Constants.COMPRESSION_TITLE));
      expCmpPanel.add(buildCmpPanel(Constants.EXPANSION_TITLE));
    ...
    /****************************************************************
     * buildExpPanel - helper method for buildExpCmpPanel() method - adds or deletes words from a
     * phrase during testing to see if the matching improves
     *
     * @return JPanel
     ****************************************************************/
    ...
    /****************************************************************
     * Create JPanel to hold the buttons to allow for either standard or no smoothing during testing
     *
     * @return JPanel
     ****************************************************************/
    private JPanel buildSmoothPanel() {
      ...
      for (String theFilter : Constants.FILTER_STRINGS) {
    ...
    /****************************************************************
     * Create and return a panel that allows user to select method of machine learning used during
     * training.
     *
     * @return JPanel
     ****************************************************************/
  ...
  protected int grpByColNum = Constants.GROUPBY_COLUMN_NUMBER_DEFAULT;

  protected int sampleSize = Constants.BOOTSTRAP_SAMPLE_SIZE_DEFAULT;
  protected double sampleFraction = Constants.BOOTSTRAP_SAMPLE_FRACTION_DEFAULT;
  private final double sequenceThreshold = Constants.SEQUENCE_PROBABILITY_THRESHOLD_DEFAULT;
  // Set default parameters
  private String curFilter = Constants.FILTER_START;
  protected String curLearnMethod = Constants.CONCEPT_MAPPING_METHOD_DEFAULT;
  private int numToCompress = Constants.NUM_COMPRESS_START;
  private int numToExpand = Constants.NUM_EXPAND_START;
  private boolean createSummary = Constants.CREATE_SUMMARY_START;
  boolean includeNulls = Constants.USE_NULLS_START;
  private int bootstrapIterations = Constants.BOOTSTRAP_ITERATIONS_DEFAULT;
  private String exportFormat = Constants.EXPORTABLE_FILETYPES[Constants.EXPORT_FORMAT_START];
  boolean useStems = Constants.USE_TOKEN_STEMMING;
  boolean usePOS = Constants.USE_PARTS_OF_SPEECH;
  boolean invertTokenSequence = Constants.INVERT_TOKEN_SEQUENCE;
  boolean removeStopWords = Constants.REMOVE_STOP_WORDS;
  private int trainingTokenNumber = Constants.MAX_TOKENS_DEFAULT;
  protected ContextHandling reasonNoMedsProcessing = Constants.REASON_NO_MEDS_PROCESSING_DEFAULT;
  protected ContextHandling negationProcessing = Constants.NEGATION_HANDLING_DEFAULT;
  private java.util.List<String> annotatedFileNames = Constants.RPT_ANNOTATED_FILENAMES;
  private java.util.List<String> annotatedFilePaths = Constants.RPT_ANNOTATED_FILEPATHS;
  private File lastSelectedDir = Constants.LAST_SELECTED_DIR;
  private boolean keepUnlabeledOnDisk = Constants.STORE_UNLABELED_ON_DISK_DURING_TRAINING;
  private boolean saveUnlabeledToDisk = Constants.SAVE_UNLABELED_TO_DISK_AFTER_TRAINING;

  protected TSFinderMethod phraseIDTrainingMethod = Constants.PHRASE_ID_TRAINING_METHOD_DEFAULT;

  protected PhraseIDSmoothingMethod phraseIDSmoothingMethod =
      Constants.SMOOTHING_FOR_PHRASE_ID_DEFAULT;
  ...
  protected int annotationSurroundingTokens = Constants.PHRASE_ID_SURROUNDING_TOKENS_DEFAULT;
  ...
  private OptimismCorrectionMethod optimismCorrection = Constants.OPTIMISM_CORRECTION_DEFAULT;


  /*****************************************************************
   * Private so that it cannot be instantiated more than once
   *****************************************************************/
  ...
  /****************************************************************
   * Set the options used for mapping phrases to concepts using a loaded solution. This is not
   * currently used.
   *
   * @param location
   * @param size
  ...
  /****************************************************************
   * Set the options used for testing the current system
   *
   * @param location
   * @param size
   ****************************************************************/
  ...
  /****************************************************************
   * Create JPanel to hold the buttons to allow or disallow some feature
   *
   * @return JPanel
   ****************************************************************/
