 ...
 *
 * @author Julien Thibault, Stephane Meystre, Oscar Ferrandez-Escamez Department of Biomedical
 *         Informatics, University of Utah, 2011
 */
public class ConText {
  ...
  // originally this pattern recognized UMLS concepts, but for this
  // application
  ...
  public ConText() {
    ...
    for (int i = 0; i < this.regexes.length; i++) {
      int attrIndex = this.regexes[i].indexOf(',');
      int attrIndex2 = this.regexes[i].lastIndexOf(',');

      String phrase = this.regexes[i].substring(0, attrIndex).replaceAll(" ", "[\\\\s\\\\-]");
      String position = this.regexes[i].substring(attrIndex + 1, attrIndex2);
      String contextType = this.regexes[i].substring(attrIndex2 + 1);

      // This adds the phrases that are considered "pseudo" to a
      // long string with '|' (ORs) to separate phrases that are pseudo.
      if (position.compareTo("pseudo") == 0) {
        ...
        // I'm not sure this can be reached
        else if (contextType.compareTo("hist") == 0) {
    ...
    if (regex_PSEUDO.length() > 0) {
      this.regexPseudo = Pattern.compile(regex_PSEUDO.substring(2));
    }

    // negation context
    if (regex_NEG_PRE.length() > 0) {
      this.regexNegPre = Pattern.compile(regex_NEG_PRE.substring(1));
    }
    if (regex_NEG_POST.length() > 0) {
      this.regexNegPost = Pattern.compile(regex_NEG_POST.substring(1));
    }
    if (regex_NEG_END.length() > 0) {
      this.regexNegEnd = Pattern.compile(regex_NEG_END.substring(1));
    }
    if (regex_POSS_PRE.length() > 0) {
      this.regexPossPre = Pattern.compile(regex_POSS_PRE.substring(1));
    }
    if (regex_POSS_POST.length() > 0) {
      this.regexPossPost = Pattern.compile(regex_POSS_POST.substring(1));
    }

    // temporality context
    if (regex_HIST_PRE.length() > 0) {
      this.regexHistPre = Pattern.compile(regex_HIST_PRE.substring(1));
    }
    if (regex_HYPO_PRE.length() > 0) {
      this.regexHypoPre = Pattern.compile(regex_HYPO_PRE.substring(1));
    }
    if (regex_HIST_1W.length() > 0) {
      this.regexHist1w = Pattern.compile(regex_HIST_1W.substring(1));
    }
    if (regex_HIST_END.length() > 0) {
      this.regexHistEnd = Pattern.compile(regex_HIST_END.substring(1));
    }
    if (regex_HYPO_END.length() > 0) {
      this.regexHypoEnd = Pattern.compile(regex_HYPO_END.substring(1));
    }

    // experiencer and mixed
    if (regex_EXP_PRE.length() > 0) {
      this.regexExpPre = Pattern.compile(regex_EXP_PRE.substring(1));
    }
    if (regex_EXP_END.length() > 0) {
      this.regexExpEnd = Pattern.compile(regex_EXP_END.substring(1));
    }
    if (regex_HYPO_EXP_END.length() > 0) {
      this.regexHypoExpEnd = Pattern.compile(regex_HYPO_EXP_END.substring(1));
    }
    if (regex_HIST_EXP_END.length() > 0) {
      this.regexHistExpEnd = Pattern.compile(regex_HIST_EXP_END.substring(1));
    }

    /**********
     * System.out.println( "-----------------------------------------------------");
     * System.out.println("NEGEX CONFIG: "); System.out.println( "\tMax window: " + MAX_WINDOW + "
     * words"); System.out.println( "\tPRENEG: " + regex_NEG_PRE.substring(1)); System.out.println(
     * "\tPOSTNEG: " + regex_NEG_POST.substring(1)); System.out.println( "\tPREPOSS: " +
     * regex_POSS_PRE.substring(1)); //System.out.println( "POSTPOSS: " +
     * regex_POSS_POST.substring(1)); System.out.println( "\tPSEUDO: " + regex_PSEUDO.substring(1));
     * System.out.println( "\tNEGTERM: " + regex_NEG_END.substring(1)); System.out.println("\n");
     **********/
  }
  ...
  public String applyExperiencer(String[] words) throws Exception {
    ...
    // Going from one experiencer term to another, and creating the
    // appropriate window
    int mm = 0;
    while (mm < words.length) {
      // IF word is a pseudo-negation, skips to the next word
      if (words[mm].equals("<NEG_PSEUDO>")) {
        mm++;
      } else if (words[mm].equals("<EXP_PRE>")) {
        // expands window until end of sentence, termination term, or
        // other negation/possible trigger term
        for (int o = 1; mm + o < words.length; o++) {
          if (words[mm + o].equals("<EXP_END>|<HIST_EXP_END>|<HYPO_EXP_END>|<EXP_PRE>")) {
            break;// window decreased to right before other negation
            // or conjunction
          } else {
        ...
        for (int w = 0; w < window.size(); w++) {
          if (window.get(w).matches(ConText.regExUmlsTag)) {
            String umlsWord = window.get(w);
            // int index =
            // Integer.parseInt(umlsWord.replaceAll("\\[|\\]",""));
  ...
  public String applyNegEx(String[] words) throws Exception {
    // Going from one negation to another, and creating the appropriate
    // window
    int m = 0;
    List<String> window = new ArrayList<>();
    ...
    while (m < words.length) {
      ...
      // IF word is a pre- concept negation or possible...
        ...
        int maxWindow = ConText.MAX_WINDOW;
        if (words.length < m + maxWindow) {
          maxWindow = words.length - m;
        }
  ...
  public String applyTemporality(String[] words) throws Exception {
    ...
    // Going from one temporality term to another, and creating the
    // appropriate window
    int mm = 0;
    while (mm < words.length) {
      // IF word is a pseudo-negation, skips to the next word
      ...
      // IF word a pre- historical trigger term
        ...
        for (int o = 1; mm + o < words.length; o++) {
          if (words[mm + o].matches("<HIST_END>|<HIST_EXP_END>|<HIST_PRE>|<HIST_1W>")) {
            break;// window decreased to right after other negation
            // or conjunction
          } else {
            window.add(words[mm + o]);
          }
  ...
  private String preProcessSentence(String sent, String concept) throws Exception {
    ...
    if (conceptIndex != -1) {
      ...
      sentenceTagged = sentenceTagged.substring(0, charOffset) + tag
          + sentenceTagged.substring(charOffset + umlsConcept.length());
    ...
    // days
    this.regexTimeFor =
        Pattern.compile("[for|over] the [last|past] (((1[4-9]|[1-9]?[2-9][0-9])[ |-][day|days] of)|"
            + "(([2-9]|[1-9][0-9])[ |-][week|weeks] of)|"
            + "(([1-9]?[0-9])[ |-][month|months|year|years] of))");// other
    // pattern
    // to
    // recognize
    // expressions
    ...
    // >14
    // days
    this.regexTimeSince = Pattern.compile("since [last|the last]? ((([2-9]|[1-9][0-9]) weeks ago)|"
    ...
    Matcher mTime = this.regexTimeFor.matcher(sentenceTagged);
    sentenceTagged = mTime.replaceAll(" <TIME_PRE> ");
    mTime = this.regexTime.matcher(sentenceTagged);
    sentenceTagged = mTime.replaceAll(" <TIME_PRE> ");
    mTime = this.regexTimeSince.matcher(sentenceTagged);
