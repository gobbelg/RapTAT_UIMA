 ...
 *
 * This program is to determine an experiencer for ConText: an algorithm for identifying contextual
 * features in textual medical records
 *
 * NOTES: returns an experiencer if any experiencer is found. Otherwise, return "Patient"
 *
 ****************************************************************************************/
...
public class GenExperiencer {
  ...
  // post: helping method for getExperiencer to actually process a line to
  // find an experiencer
  ...
  private String helper(String[] s, int index) {
    if (index < s.length) {
      for (int i = index; i < s.length; i++) {
        ...
        } else if (contains(s, this.list, i) != -1) {
          String whom = s[i];
          if (whom.contains("'s")) {
            whom = whom.replace("'s", "");
