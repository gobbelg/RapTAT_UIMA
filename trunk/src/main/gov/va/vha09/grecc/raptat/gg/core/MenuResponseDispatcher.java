...
import src.main.gov.va.vha09.grecc.raptat.gg.core.dialogs.DatabaseCreationWindow;
...
public class MenuResponseDispatcher {
  ...
  public void mapSolution(ConceptMapSolution curSolution, OptionsManager annotatorOptions) {
    ...
    if (dataToMap != null) {
      ...
      if (trainMethod.endsWith(Constants.CONCEPT_MAPPING_SOLUTIONS[0])) {
        curEvaluator = new BOWConceptMapEvaluator(curSolution, annotatorOptions);
      } else if (trainMethod.endsWith(Constants.CONCEPT_MAPPING_SOLUTIONS[1])) {
        curEvaluator = new NBEvaluator(curSolution, annotatorOptions);

      } else if (trainMethod.endsWith(Constants.CONCEPT_MAPPING_SOLUTIONS[2])) {
        curEvaluator = new NonNBEvaluator(curSolution, annotatorOptions);
      } else if (trainMethod.endsWith(Constants.CONCEPT_MAPPING_SOLUTIONS[3])) {
      ...
      if (curEvaluator != null) {
        ...
        // csv file
        if (dataToMap.getFileType().equals(Constants.IMPORTABLE_FILETYPES[1])) {
  ...
  public void runPhraseIDCrossValidationAnalysis(boolean filterAnnotations) {
    ...
    if (detailedReport != JOptionPane.CANCEL_OPTION) {
      ...
      HashSet<String> trainingConcepts =
          SchemaConcept.getConcepts(null, "Select tab-delimited file containing training concepts");
      HashSet<String> scoringConcepts =
          SchemaConcept.getConcepts(null, "Select tab-delimited file containing scoring concepts");
      ...
      boolean storeResultsAsXML = Constants.STORE_CROSS_VALIDATION_RESULTS_AS_XML;
