...
public class BOWSequenceIDSolution implements SequenceIDStructure, Serializable {
  ...
  // during training
  transient private int tokenWindowWidth = 0;
  ...
  public BOWSequenceIDSolution(PhraseIDTrainOptions trainingOptions) {
    ...
    for (int i = 0; i < this.tokenConditionals.length; i++) {
      this.tokenConditionals[i] = new ConcurrentHashMap<>(Constants.ESTIMATED_VOCABULARY_SIZE);
    }

    this.tokenWindowWidth = trainingOptions.getSurroundingTokensToUse();
    this.priors = new PriorTable(this.tokenWindowWidth + 1, Constants.MAX_TOKENS_DEFAULT + 1,
        this.tokenWindowWidth + 1, Constants.MAX_TOKENS_DEFAULT);
  }
  ...
  public void updateNonAnnotated(List<String> tokenStrings, int tokenWindowWidth) {
    ...
    this.totalTokens[BOWSequenceIDSolution.NOT_ANNOTATED][Constants.BOW_SEQUENCE_ID_PRIOR_INDEX] +=
        sentenceSize - 1;

    // The first and last tokens are added by the program and will never be
    // annotated
    // so they are subtracted from sentence size
    this.totalTokens[BOWSequenceIDSolution.NOT_ANNOTATED][Constants.BOW_SEQUENCE_ID_PHRASE_INDEX] +=
        sentenceSize - 2;

    // The first token cannot be downstream of a phrase
    this.totalTokens[BOWSequenceIDSolution.NOT_ANNOTATED][Constants.BOW_SEQUENCE_ID_POST_INDEX] +=
        sentenceSize - 1;
    ...
    incrementUnannotated(curTokenString, 1, Constants.BOW_SEQUENCE_ID_PRIOR_INDEX);

    for (i = 1; i < sentenceSize - 1; i++) {
      curTokenString = tokenStrings.get(i);
      incrementUnannotated(curTokenString, 1, Constants.BOW_SEQUENCE_ID_PHRASE_INDEX);
      incrementUnannotated(curTokenString, 1, Constants.BOW_SEQUENCE_ID_PRIOR_INDEX);
      incrementUnannotated(curTokenString, 1, Constants.BOW_SEQUENCE_ID_POST_INDEX);
    }
  ...
  /*************************************************************
   * Calculate missingTokenLikelihood, which determines contribution of tokens that are absent in
   * the training set but appear in the test set.
   *
   * @author Glenn Gobbel - Sep 28, 2012
   *************************************************************/
  private void updateMissingTokenLikelihood() {
    int curIndex = Constants.BOW_SEQUENCE_ID_PHRASE_INDEX;
    ...
    if (this.tokenWindowWidth > 0) {
      curIndex = Constants.BOW_SEQUENCE_ID_PRIOR_INDEX;
      if (this.totalTokens[BOWSequenceIDSolution.ANNOTATED][curIndex] > 0
          && this.totalTokens[BOWSequenceIDSolution.NOT_ANNOTATED][curIndex] > 0) {
        this.missingTokenLikelihood[curIndex] =
            Math.log10(this.totalTokens[BOWSequenceIDSolution.ANNOTATED][curIndex])
      ...
      curIndex = Constants.BOW_SEQUENCE_ID_POST_INDEX;
      if (this.totalTokens[BOWSequenceIDSolution.ANNOTATED][curIndex] > 0
          && this.totalTokens[BOWSequenceIDSolution.NOT_ANNOTATED][curIndex] > 0) {
        this.missingTokenLikelihood[curIndex] =
            Math.log10(this.totalTokens[BOWSequenceIDSolution.ANNOTATED][curIndex])
