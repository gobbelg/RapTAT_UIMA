...
public class CSAttributeTagger {
  ...
  public CSAttributeTagger(AttributeTaggerSolution taggerSolution, CrfSuiteRunner crfSuiteRunner) {
    ...
    contextType = taggerSolution.getContext();
    featureBuilder = (CSFeatureBuilder) taggerSolution.getFeatureBuilder();
    featureBuilder.initializeTransients();
    includedScopeFeatures = featureBuilder.getIncludedScopeFeaturesSet();
  ...
  public void assignAttribute(AnnotatedPhrase annotatedPhrase) {
    ...
    this.assignCSAttributes(phraseAsList);
  }
  ...
  public void assignCSAttributes(List<AnnotatedPhrase> annotatedPhrases) {
    ...
    this.assignCSAttributes(annotatedPhrases, sentences);
  }


  /**
   * Method used when a set of annotated phrases come from only a single sentence
   *
   * @param annotatedPhrases
   * @param associatedSentence
   */
  public void assignCSAttributes(List<AnnotatedPhrase> annotatedPhrases,
      AnnotatedPhrase associatedSentence) {
    List<AnnotatedPhrase> phraseList = new ArrayList<>();
    phraseList.add(associatedSentence);
    this.assignCSAttributes(annotatedPhrases, phraseList);
  }


  public void assignCSAttributes(List<AnnotatedPhrase> annotatedPhrases,
      List<AnnotatedPhrase> associatedSentences) {
    /*
    ...
    RaptatPair<List<ItemSequence>, List<ItemSequence>> cueAndScopeFeatures =
        featureBuilder.buildTaggingFeatures(associatedSentences);
     ...
     */
    assignSentenceTokenContext(associatedSentences, scopeLabels);
  ...
  private void addPhraseAttributes(List<AnnotatedPhrase> annotatedPhrases) {
    for (AnnotatedPhrase curPhrase : annotatedPhrases) {
      ...
      String uniqueID = "RapTAT_Attribute_" + idGenerator.getUnique();
      ...
      if (attribute == null) {
        attribute = new RaptatAttribute(uniqueID, contextType.getTypeName(),
            contextType.getDefaultValue(), AttributeAssignmentMethod.CUE_SCOPE_CRF);
      }
  ...
  private void assignSentenceTokenContext(List<AnnotatedPhrase> sentences,
    ...
    for (RaptatPair<Integer, Integer> curPair : scopeLabels) {
      ...
      tokenToAssign.setContextValue(contextType, contextType.getCommonlyAssignedValue());
    }
  }
  ...
  private int[] getCueLocations(List<Pair<String, Double>> tokenLabels) {
    ...
    while (tokenIterator.hasNext()) {
      ...
      if (curLabel.equals(Constants.BEGIN_CUE_STRING)
          || curLabel.equals(Constants.INSIDE_CUE_STRING)) {
  ...
  private List<RaptatPair<Integer, Integer>> getLabels(List<ItemSequence> cueFeatures,
    ...
    CrfTagger cueTagger = crfSuiteRunner.getCueTagger();
    CrfTagger scopeTagger = crfSuiteRunner.getScopeTagger();
  ...
  private boolean updateScopeFeatures(List<Pair<String, Double>> tokenLabels,
    ...
    HashMap<Integer, List<Integer>> cueOffsetMap =
        featureBuilder.createMapFromCueLocations(cueLocations, tokenLabels.size());
    ...
    for (Integer curTokenIndex : cueOffsetMap.keySet()) {
      ...
      for (Integer curOffset : cueOffsetMap.get(curTokenIndex)) {
        String candidateFeature = Constants.CUE_OFFSET_STRING + "@" + curOffset.toString();
        if (includedScopeFeatures.isEmpty() || includedScopeFeatures.contains(candidateFeature)) {
    ...
    SwingUtilities.invokeLater(new Runnable() {
      ...
      private String reportAttributes(AnnotatedPhrase curPhrase,
        ...
        String documentName = curPhrase.getProcessedTokens().get(0).getSentenceOfOrigin()
            .getDocumentOfOrigin().getTextSourceName().orElse("NoDocumentName");
