  ...
  public Instance pipe(Instance carrier) {
    ...
    for (RaptatToken curToken : phraseTokens) {
      ...
      if (label == null || label.equals("")) {
        label = Constants.DEFAULT_CRF_OUTSIDE_STRING;
      }
      ...
      if (label.equalsIgnoreCase(Constants.BEGIN_CUE_STRING)
          || label.equalsIgnoreCase(Constants.INSIDE_CUE_STRING)) {
    ...
    for (int i = 0; i < cuePositionList.size(); i++) {
      ...
      for (int j = 0; j < tokenList.size(); j++) {
        ...
        String cueStringFeature =
            Constants.SCOPE_FEATURE_TAG + "_" + featureTag + "_" + "CUE=" + cuePhrase;
        ...
        String cuePositionFeature =
            Constants.SCOPE_FEATURE_TAG + "_" + featureTag + "_" + "CUE_POSITION=" + distanceToCue;
        tokenList.get(j).setFeatureValue(cuePositionFeature, 1);

        String cueDistanceFeature = Constants.SCOPE_FEATURE_TAG + "_" + featureTag + "_"
            + "CUE_DISTANCE=" + Math.abs(distanceToCue);
        tokenList.get(j).setFeatureValue(cueDistanceFeature, 1);

        String cueCloseFeature = Constants.SCOPE_FEATURE_TAG + "_" + featureTag + "_" + "CLOSE_CUE="
            + (Math.abs(distanceToCue) < 3);
        tokenList.get(j).setFeatureValue(cueCloseFeature, 1);

        String cueUpStreamFeature = Constants.SCOPE_FEATURE_TAG + "_" + featureTag + "_"
        ...
        for (; betweenWordIndex < betweenEnd; betweenWordIndex++) {
          tokenList.get(j).setFeatureValue(Constants.SCOPE_FEATURE_TAG + "_" + featureTag + "_"
              + "BETWEENWORD=" + tokenList.get(betweenWordIndex).getText(), 1);
        }
      }
    }
  }
