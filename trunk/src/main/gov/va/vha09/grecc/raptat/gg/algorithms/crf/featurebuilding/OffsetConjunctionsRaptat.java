...
public class OffsetConjunctionsRaptat extends Pipe implements Serializable {
  ...
  public OffsetConjunctionsRaptat(boolean includeOriginalSingletons, Pattern featureRegex,
    ...
    maxConjunctionsLimit = maxConjunctions;
    OffsetConjunctionsRaptat.logger.setLevel(Level.OFF);
  ...
  public Instance pipe(Instance carrier) {
    ...
    /*
     * Commented this out because it was not responding properly. Might be that Mallet logger is
     * taking over here????
     */
    // if (OffsetConjunctionsRaptat.logger.getLevel().equals(Level.DEBUG)) {
    // OffsetConjunctionsRaptat.logger.debug(
    // "\n-----------------------------\nAdding offset
    // conjunctions:\n-----------------------------\n");
    // for (int i = 0; i < conjunctions.length; i++) {
    // OffsetConjunctionsRaptat.logger
    // .debug("\tOffsetArray " + i + ": " + Arrays.toString(conjunctions[i]));
    // }
    //
    ...
    for (int currentTokenIndex = 0; currentTokenIndex < tokenSequenceLength; currentTokenIndex++) {
       ...
       */
      for (int conjunctionRow = 0; conjunctionRow < conjunctions.length; conjunctionRow++) {
         ...
         */
        PropertyList.Iterator[] propertyListIterators = getOffsetIters(conjunctions, conjunctionRow,
            tokenSequenceLength, currentTokenIndex, regexMatchingFeatures);
        if (propertyListIterators == null) {
          continue;
        }
        PropertyList combinedFeatures =
            combineAllFeatures(propertyListIterators, conjunctions[conjunctionRow]);
    ...
    // if (OffsetConjunctionsRaptat.logger.isDebugEnabled()) {
    // for (int i = 0; i < conjunctions.length; i++) {
    // System.out.println("\tOffsetArray " + i + ": " + Arrays.toString(conjunctions[i]));
    // }
    // System.out.println("\n=========================\n");
  ...
  private PropertyList combineAllFeatures(PropertyList.Iterator[] propertyListIterators,
     ...
     */
    String startTag = "";
    if (maxConjunctionsLimit > 0 && elementOffsets.length > maxConjunctionsLimit
        || maxOffsetLimit > 0 && GeneralHelper.maxAbsoluteValue(elementOffsets) > maxOffsetLimit) {
      startTag = maxExceededTag + "_";
    }
  ...
  private PropertyList findMatchingProperties(PropertyList.Iterator iterator) {
    ...
    while (iterator.hasNext()) {
      ...
      // );
      if (featureRegex != null && featureRegex.matcher(feature).matches()) {
  ...
  private void writeObject(ObjectOutputStream out) throws IOException {
    ...
    int size1, size2;
    size1 = conjunctions == null ? OffsetConjunctionsRaptat.NULL_INTEGER : conjunctions.length;
    out.writeInt(size1);
    if (size1 != OffsetConjunctionsRaptat.NULL_INTEGER) {
      for (int i = 0; i < size1; i++) {
        size2 = conjunctions[i] == null ? OffsetConjunctionsRaptat.NULL_INTEGER
