...
public class ConceptMapBootstrapRunner {
  ...
  private boolean useTrainingStems = Constants.USE_TOKEN_STEMMING;
  ...
  public ConceptMapBootstrapRunner(ConceptMapBootstrapAnalyzer inputAnalyzer,
    ...
    if (curLearnMethod.endsWith(Constants.CONCEPT_MAPPING_METHODS[0])) {
    ...
    if (curLearnMethod.endsWith(Constants.CONCEPT_MAPPING_METHODS[1])) {
    ...
    } else if (curLearnMethod.endsWith(Constants.CONCEPT_MAPPING_METHODS[2])) {
      this.curTrainer = new NonNBTrainer(this.useNulls, this.useTrainingStems);
      this.curEvaluator = new NonNBEvaluator();
    } else if (curLearnMethod.endsWith(Constants.CONCEPT_MAPPING_METHODS[3])) {
