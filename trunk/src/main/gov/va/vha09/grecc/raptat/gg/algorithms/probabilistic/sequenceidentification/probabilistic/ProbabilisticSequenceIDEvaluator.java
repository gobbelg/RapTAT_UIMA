...
/***************************************************************
 * Uses a LabeledHashTreeInstance to evaluate a set of sentences and identify the sequences of
 * tokens (phrases) that should be annotated.
 *
...
public class ProbabilisticSequenceIDEvaluator extends SequenceIDEvaluator {
  ...
  protected double sequenceProbabilityThreshold = Constants.SEQUENCE_PROBABILITY_THRESHOLD_DEFAULT;
  ...
  public List<List<RaptatToken>> getSentenceAnnotations(List<RaptatToken> sentenceTokens) {
    ...
    for (int startTokenIndex = 0; startTokenIndex < sentenceTokens.size(); startTokenIndex++) {
      ...
      for (int endTokenIndex = startTokenIndex + 1; endTokenIndex <= sentenceTokens.size()
          && endTokenIndex - startTokenIndex <= Constants.MAX_TOKENS_DEFAULT
  ...
  /***************************************************************************
   * Precondition: Tokens within each sentence are already sorted by offset from low to high or high
   * to low, depending on whether the user wants to use inverted token sequences.
   *
