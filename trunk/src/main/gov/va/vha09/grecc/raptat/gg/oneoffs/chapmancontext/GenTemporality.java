...
/***************************************************************************************
 * Author: Junebae Kye, Supervisor: Imre Solti Date: 07/04/2010
 *
 * This program is to determine a temporality for ConText: an algorithm for identifying contextual
 * features in textual medical records
 *
 * NOTES: returns a temporality if any temporality is found. Otherwise, returns "Recent"
 *
 ****************************************************************************************/
...
public class GenTemporality {
  ...
  // creates lists of recent phrases, hypothetical phrases, pseudo phrases,
  // and historical phrases
  public GenTemporality() {
    this.recent = new LinkedList<>();
    this.hypothetical = new LinkedList<>();
    this.pseudo = new LinkedList<>();
    this.historical = new LinkedList<>();
    process(this.hypothetical, this.pseudo, this.historical);
  }
  ...
  // post: returns an index of temporality if a temporality phrase is found.
  // Otherwise, returns -1
  ...
  private String helper(String[] s, int index) {
    if (index < s.length) {
      for (int i = index; i < s.length; i++) {
        int indexII = contains(s, this.pseudo, i);
        if (indexII != -1) {
          return helper(s, indexII);
        } else if (contains(s, this.historical, i) != -1) {
          return "Historical";
        } else if (contains(s, this.hypothetical, i) != -1) {
          return "Hypothetical";
        }
      }
    }
  ...
  private void process(List<String> hypothetical, List<String> pseudo, List<String> historical) {
    this.recent.add("last week");
    this.recent.add("yesterday");
    this.recent.add("week ago");
    this.recent.add("last night");
    this.recent.add("the other night");
