...
public class TrainingProcessor {
  ...
  protected boolean useStems = Constants.USE_TOKEN_STEMMING;
  protected boolean usePOS = Constants.USE_PARTS_OF_SPEECH;
  ...
  protected int maxTokenNumber = Constants.MAX_TOKENS_DEFAULT;
  protected boolean useNulls = Constants.USE_NULLS_START;
  ...
  /*************************************************************
   * Constructor method used by BootstrapAnalyzer and CrossValidationAnalyzer. This constructor
   * generates unlabeled hash trees for all the training groups so they don't have to be regenerated
   * in the iterative processes involved with bootstrapping and leave one out analysis
   *
   * @param tokenOptions
  ...
  public void trainRaptat(AnnotationGroup inputGroup) {
    ...
    TrainingProcessor.logger
        .debug("Updating sequence training for:" + inputGroup.getRaptatDocument().getTextSourceName());
    this.tsiTrainer.updateTraining(inputGroup);

    TrainingProcessor.logger
        .debug("Updating concept mapping for:" + inputGroup.getRaptatDocument().getTextSourceName());
  ...
  public void trainRaptat(List<AnnotationGroup> trainingGroups, boolean saveUnlabeledToDisk,
    ...
    for (AnnotationGroup curGroup : trainingGroups) {
      TrainingProcessor.logger
          .debug("Updating sequence training for:" + curGroup.getRaptatDocument().getTextSourceName());
      this.tsiTrainer.updateTraining(curGroup);

      TrainingProcessor.logger
          .debug("Updating concept mapping for:" + curGroup.getRaptatDocument().getTextSourceName());
