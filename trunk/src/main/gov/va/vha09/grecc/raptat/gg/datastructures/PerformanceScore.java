...
public class PerformanceScore implements Comparable {
  ...
  public int compareTo(Object other) {
    ...
    if (other instanceof PerformanceScore) {
      ...
      if (otherScore.concept.equals(Constants.TOTAL_PARTIAL_MATCH_NAME)) {
        return AFTER;// Partial phrase match comes first
      }
      if (this.concept.equals(Constants.TOTAL_PARTIAL_MATCH_NAME)) {
        return BEFORE;// Partial phrase match comes first
      }
      if (otherScore.concept.equals(Constants.TOTAL_EXACT_MATCH_NAME)) {
        return AFTER;// Exact phrase matches comes next
      }
      if (this.concept.equals(Constants.TOTAL_EXACT_MATCH_NAME)) {
        return BEFORE;// Partial phrase match comes first
      }
