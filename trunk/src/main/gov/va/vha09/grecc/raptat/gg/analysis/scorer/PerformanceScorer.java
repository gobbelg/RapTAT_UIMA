...
public class PerformanceScorer {
  ...
  private final static int TP = 0, FP = 1, FN = 2;
  ...
  // phrases.
  private final static int NO_OVERLAP_SCORE = Integer.MIN_VALUE;
   ...
   */
  private final static int CONCEPT_MISMATCH_SCORE =
  ...
  private Constants.ScoringMode scoringMode = Constants.ScoringMode.CONCEPT_AND_ATTRIBUTE_SCORING;
  ...
  public PerformanceScorer() {
    PerformanceScorer.LOGGER.setLevel(Level.OFF);
  }
   ...
   */
  public PerformanceScorer(HashSet<String> acceptedConcepts, int maxPhraseTokens) {
    this();
   ...
   */
  public PerformanceScorer(HashSet<String> acceptedConcepts, int maxPhraseTokens,
      int conceptMatchWeight, int attributeMatchWeight, Constants.ScoringMode scoringMode) {
    this();
    this.maxPhraseTokens = maxPhraseTokens;
    this.acceptedConcepts = acceptedConcepts;
    weightOfAttributeMatch = attributeMatchWeight;
    weightOfConceptMatch = conceptMatchWeight;
   ...
   * @param scoringMode The scoring method
   */
  public PerformanceScorer(int conceptMatchWeight, int attributeMatchWeight,
      Constants.ScoringMode scoringMode) {
    this();
    weightOfAttributeMatch = attributeMatchWeight;
    weightOfConceptMatch = conceptMatchWeight;
    this.scoringMode = scoringMode;
   ...
   * @author Glenn Gobbel - Jan 21, 2014
   */
  public PerformanceScorer(String fileDirectory, boolean createDetailedReport,
      String settingsFileTag) {
    this();
   ...
   *         **********************************************************
   */
  public float calculateF_Measure(float precision, float recall) {
   ...
   * @author Developed by Shrimalini Jayaramaraja on August 16th 2011
   */
  public float calculatePrecision(float numOfTP, float numOfFP) {
    @SuppressWarnings("UnusedAssignment")
    float precision = 0;
   ...
   *         ************************************************************
   */
  public float calculateRecall(float numOfTP, float numOfFN) {
    @SuppressWarnings("UnusedAssignment")
    float recall = 0;
  ...
  public void closeWriter() {
    if (annotationComparisonWriter != null) {
      annotationComparisonWriter.close();
    }
  }
  ...
  public List<RaptatPair<String, Integer>> compareConceptAttributeValues(
      AnnotatedPhrase raptatPhrase, AnnotatedPhrase referencePhrase) {
    ...
    for (String attributeName : raptatAttributeValueMap.keySet()) {
      ...
      if (referenceAttributeValueMap.containsKey(attributeName)) {
        ...
        for (String value : raptatAttributeValueMap.get(attributeName)) {
          StringBuilder sb =
              new StringBuilder(conceptName).append(Constants.DELIMITER_PERFORMANCE_SCORE_FIELD);
          sb.append(attributeName).append(Constants.DELIMITER_PERFORMANCE_SCORE_FIELD);
          sb.append(value);
  ...
  public HashMap<String, AnnotationOverlapGraphEdge> getMentionIdToAnnotationGraphEdgeMap() {
    return mentionIdToAnnotationGraphEdgeMap;
  }
  ...
  public Hashtable<PerformanceScoreObject, int[]> getPerformanceScore(
      List<AnnotatedPhrase> raptatAnnotations, List<AnnotatedPhrase> referenceAnnotations) {
        ...
        if (tokenOverlap > -1) {
          ...
          if (!curReferenceNode.partialMatch && !curRaptatNode.partialMatch) {
            ...
            if (tokenOverlap > 0 && !curReferenceNode.exactMatch && !curRaptatNode.exactMatch) {
              curScore = performance.get(Constants.TOTAL_EXACT_MATCH_NAME);
              curScore[PerformanceScorer.TP]++;
  ...
  public Hashtable<String, int[]> getPerformanceScoreForActiveLearning(
    ...
    performance.put(Constants.TOTAL_EXACT_MATCH_NAME, new int[] {0, 0, 0});
    performance.put(Constants.TOTAL_PARTIAL_MATCH_NAME, new int[] {0, 0, 0});
    ...
    for (AnnotationScorerNode curRaptatNode : raptatScorerList) {
      ...
      for (AnnotationScorerNode curReferenceNode : referenceScorerList) {
        ...
        if (tokenOverlap > -1) {
          ...
          if (!curReferenceNode.partialMatch && !curRaptatNode.partialMatch) {
            curScore = performance.get(Constants.TOTAL_PARTIAL_MATCH_NAME);
            curScore[PerformanceScorer.TP]++;
  ...
  public Hashtable<PerformanceScoreObject, int[]> getRunningSummedResult() {
    return runningSummedResult;
  }


  public List<AnnotatedPhrase> processAnnotatedPhrase(List<AnnotatedPhrase> annPhrase) {
    ...
    for (AnnotationGroup group : annotationGroups) {
      ...
      if (annotationComparisonWriter != null) {
        writeDetailedPerformanceScore(group.raptatAnnotations, group.referenceAnnotations,
            group.getRaptatDocument().getTextSourceName().orElse("NoTextSourcePath"));
      }
      summedResult = mergeResults(curResult, summedResult);
    }
    runningSummedResult = mergeResults(summedResult, runningSummedResult);
    System.out.println("Cumulative result:");
    this.print(runningSummedResult);
    if (PerformanceScorer.LOGGER.getLevel().equals(Level.DEBUG)) {
  ...
  public Hashtable<PerformanceScoreObject, int[]> scorePerformanceWithGraph(
      List<AnnotationGroup> annotationGroups, List<SchemaConcept> schemaConceptList) {
    /*
    ...
    for (AnnotationGroup annotationGroup : annotationGroups) {
      ...
      PerformanceScorer.LOGGER.info("Scoring:" + raptatDocument.getTextSourceName());
      /*
      ...
      if (PerformanceScorer.LOGGER.getLevel().equals(Level.DEBUG)) {
        writeResultsToConsole(psoToPerformanceMap,
            raptatDocument.getTextSourceName().orElse("NoTextSource"));
      }

      if (annotationComparisonWriter != null) {
        writeDetailedPerformanceScoreNew(raptatIdToAnnotationMap, referenceIdToAnnotationMap,
            schemaConceptMap, raptatDocument.getTextSourceName().orElse("NoTextSource"));
      }
    }

    runningSummedResult = mergeResults(psoToPerformanceMap, runningSummedResult);
   ...
   */
  @SuppressWarnings("UnusedAssignment")
  public void writeResultsSimple(Hashtable<String, int[]> resultMap, List<String> textSources)
      throws IOException {
    ...
    out = new BufferedWriter(new FileWriter(scoreSaveDirectory));
   ...
   */
  @SuppressWarnings("UnusedAssignment")
  public void writeResultsToConsole(Hashtable<PerformanceScoreObject, int[]> performance,
  ...
  private void addAttributesAndValuesToPerformanceMap(
      Hashtable<PerformanceScoreObject, int[]> performanceMap, SchemaConcept schemaConcept,
      String conceptName) {
  ...
  private void addRelationshipsToPerformanceMap(
      Hashtable<PerformanceScoreObject, int[]> conceptToPerformanceMap,
  ...
  private void addScoreToPerformanceMap(
      ...
      Hashtable<String, AnnotationScorerNode> raptatIdToAnnotationMap,
      Hashtable<String, AnnotationScorerNode> referenceIdToAnnotationMap,
      List<AnnotationOverlapGraphEdge> annOverlapGraphEdges,
  ...
  private List<AnnotationOverlapGraphEdge> createConnectionGraphAndMap(
    ...
    for (AnnotatedPhrase raptatPhrase : raptatAnnotations) {
      ...
      for (AnnotatedPhrase referencePhrase : referenceAnnotations) {
        ...
        if (PerformanceScorer.LOGGER.getLevel().equals(Level.DEBUG) & connectionGraph[i][j] > -1) {
    ...
    mentionIdToAnnotationGraphEdgeMap = new HashMap<>();
     ...
     */
    for (AnnotationOverlapGraphEdge edge : annOverlapGraphEdges) {
      mentionIdToAnnotationGraphEdgeMap.put(edge.getRefNodeMentionID(), edge);
      mentionIdToAnnotationGraphEdgeMap.put(edge.getRaptatNodeMentionID(), edge);
    }
   ...
   * @return
   */
  private List<AnnotatedPhrase> filterForHeadPhrases(List<AnnotatedPhrase> referenceAnnotations) {
  ...
  private int getMatchingScoreBetweenAnnotations(AnnotatedPhrase raptatPhrase,
      AnnotatedPhrase referencePhrase, HashMap<String, SchemaConcept> schemaConceptMap) {
    ...
    if (this.getTokenOverlap(raptatPhrase, referencePhrase) < 0) {
       ...
       */
      if (scoringMode == Constants.ScoringMode.ATTRIBUTE_ONLY) {
        return 0;
      }
    ...
    if (scoringMode != Constants.ScoringMode.ATTRIBUTE_ONLY) {
      score += weightOfConceptMatch;
    } else {
      weightOfAttributeMatch = 1;
    }
  ...
  private Hashtable<String, AnnotationScorerNode> getMentionIDToAnnotationNodeMap(
      List<AnnotationScorerNode> raptatNodes) {
   ...
   */
  private boolean getPerformanceAttributeScore(AnnotationScorerNode curReferenceNode,
      AnnotationScorerNode curRaptatNode, Hashtable<PerformanceScoreObject, int[]> performance,
      int tokenOverlap, int[] curScore) {
   ...
   */
  private int getTokenOverlap(AnnotatedPhrase phrase1, AnnotatedPhrase phrase2) {
   ...
   */
  @SuppressWarnings("null")
  private int getTokenOverlap(List<RaptatToken> tokenList1, List<RaptatToken> tokenList2) {
  ...
  private void print(Hashtable<PerformanceScoreObject, int[]> result) {
  ...
  private void printTabDelimited(Hashtable<String, int[]> scores, String outputFilePath) {
    PrintWriter pw = null;
    try {
  ...
  @Deprecated // Called only by Garvin create project, bootstrap
              // analysis, and
              // active learning
  private void scoreNode(Hashtable<PerformanceScoreObject, int[]> performance,
      AnnotationScorerNode curNode, int elementToScore) {
    int[] curScore;
  ...
  private void scoreNodeForActiveLearning(Hashtable<String, int[]> performance,
      AnnotationScorerNode curNode, int elementToScore) {
    int[] curScore;
  ...
  @Deprecated // Called only by Garvin create project, bootstrap
              // analysis, and
              // active learning
  private void updatePerformance(Hashtable<PerformanceScoreObject, int[]> performance,
      ArrayList<AnnotationScorerNode> raptatScorerList,
      ArrayList<AnnotationScorerNode> referenceScorerList) {
    if (PerformanceScorer.LOGGER.getLevel().equals(Level.DEBUG)) {
      System.out.println("\nFALSE POSITIVES");
      System.out.println("-----------------");
  ...
  private void updatePerformanceForActiveLearning(Hashtable<String, int[]> performance,
      ArrayList<AnnotationScorerNode> raptatScorerList,
      ArrayList<AnnotationScorerNode> referenceScorerList) {
  ...
  private void updatePerformanceScoreForConcept(
      Hashtable<PerformanceScoreObject, int[]> performance, AnnotationScorerNode node,
      int scoreToUpdate, HashMap<String, SchemaConcept> schemaConceptMap) {
    int[] score;
    ...
    PerformanceScoreObject partialMatchPSO =
        new PerformanceScoreObject(Constants.TOTAL_PARTIAL_MATCH_NAME);
    score = performance.get(partialMatchPSO);
    score[scoreToUpdate]++;

    PerformanceScoreObject totalMatchPSO =
        new PerformanceScoreObject(Constants.TOTAL_EXACT_MATCH_NAME);
   ...
   */
  private void updatePsoToScoreMapForRelationship(AnnotatedPhrase annotatedPhrase,
      Hashtable<String, AnnotationScorerNode> mentionIdToAnnotationMap,
      Hashtable<PerformanceScoreObject, int[]> performance, String conceptRelationName,
      String linkedToMentionId, int scoreIndex) {
   ...
   */
  private void updateSummaryPerformanceAttributeScore(AnnotationScorerNode referenceNode,
      int tokenOverlap, Hashtable<PerformanceScoreObject, int[]> performance,
      HashMap<String, SchemaConcept> schemaConceptMap) {
    int[] score;
    RaptatAttribute refAttribute;
    RaptatAttribute raptatAttribute;
    ...
    for (String attributeName : theSchemaConcept.getAttributeValueMap().keySet()) {
      ...
      if (refAttribute == null && raptatAttribute == null) {
        PerformanceScoreObject nullPSO = new PerformanceScoreObject(conceptName, attributeName,
            Constants.NULL_ATTRIBUTE_VALUE, null, null, null, null);
   ...
   * @return
   */
  private void updateSummaryPerformanceConceptScore(String conceptName, int tokenOverlap,
      Hashtable<PerformanceScoreObject, int[]> performance) {
    int[] score;
   ...
   * @param schemaConceptMap
   */
  private void updateSummaryPerformanceRelationshipScore(AnnotatedPhrase referencePhrase,
      AnnotatedPhrase raptatPhrase,
      Hashtable<String, AnnotationScorerNode> referenceIdToAnnotationMap,
      Hashtable<String, AnnotationScorerNode> raptatIdToAnnotationMap,
      Hashtable<PerformanceScoreObject, int[]> performance) throws Exception {
    ...
    for (ConceptRelation conceptRelation : referencePhrase.getConceptRelations()) {
      ...
      if (connectionEdge == null) {
        updatePsoToScoreMapForRelationship(referencePhrase, referenceIdToAnnotationMap, performance,
            conceptRelationName, linkedToMentionId, Constants.TrueFalseScores.FN.ordinal());
      } else {
    ...
    for (ConceptRelation conceptRelation : raptatPhrase.getConceptRelations()) {
      ...
      if (connectionEdge == null) {
        updatePsoToScoreMapForRelationship(raptatPhrase, raptatIdToAnnotationMap, performance,
            conceptRelationName, linkedToMentionId, Constants.TrueFalseScores.FP.ordinal());
      } else {
    ...
    for (RelationshipScorerNode node : truePositiveRelations) {
      ...
      int[] score = performance.get(pso);
      score[Constants.TrueFalseScores.TP.ordinal()]++;
    }
    ...
    for (RelationshipScorerNode node : raptatRelations) {
      ...
      PerformanceScoreObject pso = new PerformanceScoreObject(fromConcept, null, null,
          node.relationshipName, toConcept, null, null);
      int[] score = performance.get(pso);
      score[Constants.TrueFalseScores.FP.ordinal()]++;
    }
    ...
    for (RelationshipScorerNode node : falseNegativeReferenceRelations) {
      ...
      PerformanceScoreObject pso = new PerformanceScoreObject(fromConcept, null, null,
          node.relationshipName, toConcept, null, null);
      int[] score = performance.get(pso);
      score[Constants.TrueFalseScores.FN.ordinal()]++;
    }
  }
   ...
   * @param schemaConceptMap The map containing all concepts
   */
  private void updateSummaryPerformanceScore(AnnotationScorerNode referenceNode,
      AnnotationScorerNode raptatNode,
      Hashtable<String, AnnotationScorerNode> referenceIdToAnnotationMap,
      Hashtable<String, AnnotationScorerNode> raptatIdToAnnotationMap,
      Hashtable<PerformanceScoreObject, int[]> psoToPeformanceMap, int conceptMatchValue,
      HashMap<String, SchemaConcept> schemaConceptMap) {
   ...
   */
  @Deprecated
  private void writeDetailedPerformanceScore(List<AnnotatedPhrase> raptatAnnotations,
      List<AnnotatedPhrase> referenceAnnotations, String docSource) {
    ArrayList<AnnotationScorerNode> raptatScorerList = createScorerNodeList(raptatAnnotations);
    ArrayList<AnnotationScorerNode> referenceScorerList =
        createScorerNodeList(referenceAnnotations);
      ...
      for (AnnotationScorerNode curReferenceNode : referenceScorerList) {
        ...
        if (!curReferenceNode.exactMatch && !curReferenceNode.partialMatch) {
          ...
          if (tokenOverlap > -1) // At least partial overlap
          {
            ...
            annotationComparisonWriter.writeOverlapScore(curRaptatNode, curReferenceNode,
    ...
    annotationComparisonWriter.writeCompleteFalsePositives(raptatScorerList, docSource);
    annotationComparisonWriter.writeCompleteFalseNegatives(referenceScorerList, docSource);
  }
  ...
  private void writeDetailedPerformanceScoreNew(
    ...
    for (AnnotationScorerNode referenceNode : referenceAnnotationMap.values()) {
      annotationComparisonWriter.writeReferenceNodeResult(this, referenceNode, raptatAnnotationMap,
          referenceAnnotationMap, schemaConceptMap, docSource);
    }
    ...
    for (AnnotationScorerNode raptatNode : raptatAnnotationMap.values()) {
      ...
      if (!raptatNode.partialMatch) {
        annotationComparisonWriter.writeRaptatNodeResult(raptatNode, docSource);
      }
    }
   ...
   */
  protected void print(List<PerformanceScore> performanceScores) {
  ...
  public static Hashtable<PerformanceScoreObject, int[]> compareFileSets(File textFileDirectory,
      File referenceFileDirectory, File raptatFileDirectory, File schemaConceptFile,
      File acceptableConceptsFile, AnnotationApp annotationApp, boolean removeUnmatched) {
      ...
      public void run() {
        ...
        BootstrapResultWriter resultWriter =
            new BootstrapResultWriter(textDirectory.getAbsolutePath(), "TestData",
                Constants.CROSS_VALIDATION_CONCEPT_RESULT_TITLES);
  ...
  private static AnnotationGroup createAnnotationGroup(List<SchemaConcept> schemaConcepts,
      HashSet<String> acceptedConcepts, TextAnalyzer textAnalyzer,
      PhraseTokenIntegrator tokenIntegrator, AnnotationImporter annotationImporter,
      File referenceFile, File raptatFile, File textFile) {
    /*
  ...
  private static List<AnnotationGroup> createAnnotationGroups(File textFileDirectory,
      File referenceFileDirectory, File raptatFileDirectory, List<SchemaConcept> schemaConcepts,
      HashSet<String> acceptedConcepts, AnnotationApp annotationApp, boolean removeUnmatched) {
